...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="+af-packet control-socket -cuda -debug +detection geoip hardened logrotate lua luajit nflog +nfqueue redis -rules test -profiling"
...
src_install() {
	...
	insinto "/etc/${PN}"
	doins threshold.config suricata.yaml
	doins etc/{classification,reference}.config
	...
	if use logrotate; then
		...
		newins "${FILESDIR}"/${PN}-01.logrotate.conf ${PN}
	fi
}
